...
// ------------------------------------------------------------
// All client-side aggregation. user_id filtering is handled by /api/db
// which reads the auth cookie. Perf is acceptable for current scale.
// ------------------------------------------------------------

// Minimal type covering the subset of the Supabase client API used here
type DB = {
  from: (table: string) => {
    select: (cols?: string, opts?: Record<string, unknown>) => unknown
