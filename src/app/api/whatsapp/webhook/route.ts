...
export async function GET(request: Request) {
  try {
    ...
    console.log('[webhook] GET verification — mode:', mode, '| token:', verifyToken);
    ...
    // ── Fast path: check env var first (works without a saved DB config) ──
    const envToken = process.env.WHATSAPP_VERIFY_TOKEN;
    if (envToken && verifyToken === envToken) {
      console.log('[webhook] ✅ Verified via WHATSAPP_VERIFY_TOKEN env var');
    ...
    for (const config of configs) {
      if (!config.verify_token || typeof config.verify_token !== 'string')
        continue;
      try {
    ...
    if (matchedConfig) {
      ...
      console.log('[webhook] ✅ Verified via DB config');
      return new Response(challenge, {
        status: 200,
        headers: { 'Content-Type': 'text/plain' },
      });
...
async function processWebhook(body: { entry?: WhatsAppWebhookEntry[] }) {
  ...
  for (const entry of body.entry) {
    for (const change of entry.changes) {
      ...
      const decryptedAccessToken = decrypt((config as any).access_token);
      const userId =
      ...
      for (let i = 0; i < value.messages.length; i++) {
        ...
        await processMessage(message, contact, userId, decryptedAccessToken);
      }
    }
  }
}
